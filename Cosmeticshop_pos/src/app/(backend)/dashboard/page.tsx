...
export default function AnalyticsDashboard() {
  const { analytics, isLoading, error, period, setPeriod, refreshAnalytics } =
    useAnalytics();
              ...
              <div>
                ...
                {analytics?.summary.revenueGrowth && (
                  <p className="text-xs text-green-600">
                    +{formatPercent(analytics?.summary.revenueGrowth)} from last
                    period
                  </p>
                )}
              </div>
          ...
          <CardBody className="p-4">
            <div className="flex items-center">
              ...
              <div>
                ...
                {analytics?.summary.totalProfit && (
                  <p className="text-xl font-bold">
                    {formatCurrency(analytics.summary.totalProfit)}
                  </p>
                )}
                {analytics?.summary.profitGrowth && (
                  <p className="text-xs text-green-600">
                    +{formatPercent(analytics.summary.profitGrowth)} from last
                    period
                  </p>
                )}
              </div>
          ...
          <CardBody className="p-4">
            <div className="flex items-center">
              ...
              <div>
                ...
                {analytics?.summary.totalItemsSold && (
                  <p className="text-xs text-green-600">
                    +{formatPercent(analytics.summary.itemsGrowth)} from last
                    period
                  </p>
                )}
              </div>
            ...
            <div className="flex items-center">
              ...
              <div>
                ...
                {analytics?.summary.customersGrowth && (
                  <p className="text-xs text-green-600">
                    +{formatPercent(analytics.summary.customersGrowth)} from
                    last period
                  </p>
                )}
              </div>
                ...
                <Pie
                  data={
                    analytics?.paymentMethods?.map((method) => ({
                      name: method.name,
                      value: method.total, // Map to 'value' for Recharts Pie standard
                    })) || []
                  }
                  cx="50%"
                  cy="50%"
                  ...
                  outerRadius={80}
                  fill="#8884d8"
                  dataKey="value" // Updated to match mapped 'value'
                >
          ...
          <CardBody>
            <div className="space-y-4">
              ...
              <div className="flex justify-between">
                ...
                <span className="font-medium">
                  {analytics.customerInsights.averageOrdersPerCustomer.toFixed(
                    1
                  )}
                </span>
              </div>
              <div className="flex justify-between">
                ...
                <span className="font-medium">
                  {formatPercent(analytics.customerInsights.retentionRate)}
                </span>
              </div>
            </div>
          </CardBody>
          ...
          <CardBody>
            <div className="space-y-4">
              <div className="flex justify-between">
                ...
                <span className="font-medium">
                  {formatCurrency(analytics.salesPerformance.averageSaleValue)}
                </span>
              </div>
              <div className="flex justify-between">
                <span className="text-sm text-gray-500">Conversion Rate</span>
                <span className="font-medium">
                  {formatPercent(analytics.salesPerformance.conversionRate)}
                </span>
              </div>
              <div className="flex justify-between">
                <span className="text-sm text-gray-500">Peak Hour</span>
                <span className="font-medium">
                  {analytics.salesPerformance.peakHour}:00
                </span>
              </div>
              <div className="flex justify-between">
                <span className="text-sm text-gray-500">Best Day</span>
                <span className="font-medium">
                  {analytics.salesPerformance.bestDay}
                </span>
              </div>
            </div>
          </CardBody>
          ...
          <CardBody>
            <div className="space-y-4">
              <div className="flex justify-between">
                ...
                <span className="font-medium">
                  {analytics.productInsights.topCategory}
                </span>
              </div>
              <div className="flex justify-between">
                <span className="text-sm text-gray-500">Low Stock Items</span>
                <span className="font-medium">
                  {analytics.productInsights.lowStockCount}
                </span>
              </div>
              <div className="flex justify-between">
                <span className="text-sm text-gray-500">Out of Stock</span>
                <span className="font-medium">
                  {analytics.productInsights.outOfStockCount}
                </span>
              </div>
              <div className="flex justify-between">
                <span className="text-sm text-gray-500">
                ...
                <span className="font-medium">
                  {formatPercent(analytics.productInsights.averageProfitMargin)}
                </span>
              </div>
            </div>
          </CardBody>
